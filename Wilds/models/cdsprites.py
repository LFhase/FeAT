...
class Model(nn.Module):
    ...
    def getDataLoaders(args, device):
        ...
        D = DspritesDataset(os.path.join(args.data_dir, 'cdsprites'), split=False, drop_last=not args.no_drop_last)
        train_dataset = CDsprites_Dataset(D, args.data_dir, batch_size, num_domains, 'train', drop_last=not args.no_drop_last)
        val_dataset = CDsprites_Dataset(D, args.data_dir, batch_size, num_domains, 'val', drop_last=not args.no_drop_last)
        test_dataset = CDsprites_Dataset(D, args.data_dir, batch_size, num_domains, 'test', drop_last=not args.no_drop_last)

...
def ifat_train(mlp, topmlp, steps, envs, test_envs, lossf, \
    ...
    emp_envs_pos = []
    emp_envs_neg = []
    hidden_dim = topmlp.lin1.weight.size(-1)
    n_targets = topmlp.lin1.weight.size(0)
    rounds =  hparams['rounds']
    steps_per_round =  hparams['steps_per_round']
    ...
    for round_i in range(rounds):
        ...
        topmlp = TopMLP(hidden_dim = hidden_dim, n_top_layers=1, n_targets=n_targets).cuda()
        trainable_params = [var for var in mlp.parameters()]+[var for var in topmlp.parameters()]
        optimizer = optim.Adam(trainable_params, lr=lr)  
        for step  in range(steps_per_round):
            erm_losses = []
            loss = 0
            if False and len(classifiers) > 0:
                with torch.no_grad():
                    overall_feats = mlp(x)
            else:
                overall_feats = mlp(x)
            overall_logits = topmlp(overall_feats)
            erm_losses.append(lossf(overall_logits,y))
            erm_losses_retain = []
            retain_acc = 0
            for (env_i,env) in enumerate(emp_envs_pos):
                logits = topmlp(overall_feats[emp_envs_neg[env_i]])
                erm_losses.append(lossf(logits,y[emp_envs_neg[env_i]])) # augment
                per_logits = classifiers[env_i](mlp(x[emp_envs_pos[env_i]]))
                erm_losses_retain.append(lossf(per_logits,y[env])) # retain
                retain_acc += mean_accuracy(per_logits,y[emp_envs_pos[env_i]])/round_i
            loss = max(erm_losses)
            erm_losses_retain.append(loss)
            if len(erm_losses_retain)>1:
                erm_losses_retain = torch.stack(erm_losses_retain)
                loss = max(erm_losses)+erm_losses_retain[:-1].mean()/100
            weight_norm = 0
            ...
            if step % eval_steps == 0:
                ...
                with torch.no_grad():
                    for j, env in enumerate(envs + test_envs):
                        logits = topmlp(mlp(env['images']))
                        loss =  lossf(logits,env['labels'])
                        acc = mean_accuracy(logits,env['labels'])
                ...
                train_loss, train_acc, test_worst_loss, test_worst_acc = train_loss.detach().cpu().numpy(), \
                                                                            ...
                                                                            test_worst_loss.detach().cpu().numpy(),\
                                                                            test_worst_acc.detach().cpu().numpy()
                log = [np.int32(step), torch.stack(erm_losses).detach().cpu().numpy(),
                    train_loss, train_acc,\
                    train_penalty.detach().cpu().numpy(),\
                    np.array([train_penalty.item()]),test_worst_loss, test_worst_acc]
                logs.append(log)
                
                if verbose:
                    retain_loss = 0 if len(erm_losses_retain)==1 else erm_losses_retain[:-1].mean()
                    print(f"round {round_i} {retain_acc} {retain_loss}",end=" ")
                    pretty_print(*log)

        print(f"Round {round_i} last train acc {train_acc}")
        if train_acc.item()+retain_acc < 1.30:
            print("Early stop with retain acc; Load last trained weights")
            mlp.load_state_dict(best_mlp)
            topmlp.load_state_dict(best_topmlp)
            break
        else:
            best_mlp = copy.deepcopy(mlp.state_dict())
            best_topmlp = copy.deepcopy(topmlp.state_dict())
            with torch.no_grad():
                topmlp.eval()
                if len(classifiers) > 0:
                    for clf in classifiers:
                        topmlp.lin1.weight += clf.lin1.weight
                        topmlp.lin1.bias += clf.lin1.bias
                    topmlp.lin1.weight /= len(classifiers)
                    topmlp.lin1.bias /= len(classifiers)
                overall_logits = topmlp(mlp(x))
                group, _ = correct_pred(overall_logits, y)
                print(sum(group))
                confidence = F.sigmoid(overall_logits.abs()).view(-1).cpu().numpy()
                group *= confidence>0.5
                all_idx = np.arange(x.size(0))
                print(sum(group))
                emp_envs_pos=[list(all_idx[group])]
                emp_envs_neg =[list(all_idx[~group])]
                classifiers = [topmlp]

    penalty_anneal_iters = 0
    topmlp.train()
    stage2_methods = hparams['stage2_methods']
    print(f"Switching to {stage2_methods}")
    stage2_train = eval("%s_train" % stage2_methods)
    return stage2_train(mlp, topmlp, steps, envs, test_envs, lossf, \
    penalty_anneal_iters, penalty_term_weight, anneal_val, \
